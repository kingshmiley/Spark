  ...
  if (pageJumps.length === 1) {
    return (
      <button
        ...
        className="text-xs text-ink/25 hover:text-accent/70 border border-surface-border hover:border-accent/30 rounded px-1.5 py-0.5 transition-all"
        title="Jump to page"
      >
        {pageJumps[0].label}
      </button>
...
function PrintingPicker({ card, onClose }: { card: PrintCard; onClose: () => void }) {
  ...
  React.useEffect(() => {
    const name = card.scryfallData?.name ?? card.displayName
    bridge.scryfallSearch(`!"${name}"`).then((result) => {
      setLoading(false)
    ...
    >
      ...
      <div className="flex items-center gap-2 p-2">
        ...
        <button
          {...attributes}
          {...listeners}
          className="flex-shrink-0 text-ink/10 hover:text-ink/30 cursor-grab active:cursor-grabbing px-0.5 touch-none transition-colors text-base leading-none"
        ...
        <div className="flex-1 min-w-0">
          ...
          {card.scryfallData && (
            <p className="text-ink/25 text-xs truncate">{card.scryfallData.set_name} #{card.scryfallData.collector_number}</p>
          )}
          {/* Quantity + page badge */}
          <div className="flex items-center gap-2 mt-1.5">
            <div className="flex items-center rounded border border-surface-border overflow-hidden w-fit">
              <button
                ...
                className="w-5 h-5 text-ink/40 hover:text-ink hover:bg-surface-hover disabled:opacity-20 text-xs leading-none transition-colors"
              >−</button>
              <span className="text-ink/70 text-xs w-5 text-center border-x border-surface-border">{card.quantity}</span>
              <button
                onClick={() => setCardQuantity(card.id, card.quantity + 1)}
                className="w-5 h-5 text-ink/40 hover:text-ink hover:bg-surface-hover text-xs leading-none transition-colors"
              >+</button>
            </div>
        ...
        <button
          ...
          className="flex-shrink-0 w-5 h-5 flex items-center justify-center text-ink/10 hover:text-red-400 hover:bg-red-400/10 rounded transition-all text-sm leading-none"
          title="Remove"
        >×</button>
      </div>
          ...
          <button
            ...
            className={`text-xs border rounded px-1.5 py-0.5 transition-all ${
              showPrintingPicker
                ...
                : 'border-surface-border text-ink/30 hover:text-ink/60 hover:border-surface-hover'
            }`}
          >printing</button>
        )}
        <button
